...
const starts = async (simple = new WAConnection()) => {
 ...
 simple.browserDescription = [ 'Mysql Bot V3', 'Browser', '3.0' ]
console.log(color('?Allo Ges'))
console.log(color('TNGKYIU TOO'))
console.log(color(' >>>>>>>>>>>>>>>>>>>?|||||||||||||'))
console.log(color('? Allah Swt                '))
console.log(color('? Mak               '))
console.log(color('? Ayah                '))
console.log(color('? Kwn                      '))
console.log(color('  >>>>>>>>>>>>>>>??????>>||||||||||||'))
    console.log(banner)
    simple.on('qr', () => {
    ...
    simple.on('open', () => {
        success('2', 'Done Udah Nyambung  Yaaa' )
    })
...
simple.sendMessage(anu.jid, "_*Makasih Udah Add Saya, Hay Semua Saya Adalah Mysql Bot V3 Silahkan ketik .menu Untuk Memulai*_")
}
...
sendButImage(anu.jid, teks, `© Di Buat Oleh : F.W.D `, img,but = [{buttonId:`hallo`, 
...
img = await getBuffer(ppUrl)
teks = `Bye 👋@${num.split('@')[0]}\nSemoga Amal Ibadahnya Si Terima Di sisi Tuhan Yang Maha Esa Selamat Tinggal Di Group: ${mdata.subject}`
sendButImage(anu.jid, teks, `© Di Buat Oleh : F.W.D
k`, img,but= [{buttonId: `byebye`, buttonText: {displayText: `See You Broo?👋!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, number]}, thumbnail: Buffer.alloc(0)})
