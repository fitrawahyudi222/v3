...
module.exports = simple = async (simple, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6285669011805@s.whatsapp.net`]
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		faketeks = '©〖F.W.D〗' //Ganti Aja Nama Luh
		const isUrl = (url) => {
...
//SET THUMB>>>>>>>>>>>>>>>>>>>>>>>>>>>\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        const ikal = fs.readFileSync ('./poto/NATA.png')
...
//━━━━━━━━━━━━━━━[ BUTTON ]━━━━━━━━━━━━━━━━━//
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = simple.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "List Menu","footerText": `_HELLO KAK_ >.<
✋ 
...
"thumbnail": fs.readFileSync('./poto/YENI.jpeg')
}
                ...
                quoted: {
                    ...
                    message: {
                        "imageMessage": {
                            ...
                            "jpegThumbnail": fs.readFileSync('./poto/YENI.jpeg'),
        ...
        const fakeitem = (teks) => {
           simple.sendMessage(from, teks, text, {
                   quoted: {
                           ...
                           },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync("./poto/YENI.jpeg"),"itemCount":128,"status":"INQUIRY","surface":"CATALOG","message": `🐣 JAM : ${time}`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},
...
_*JIKA BUTTON TIDAK TERLIHAT KETIK .allmenu*_
 ...
 but = [
          ...
          { buttonId: `${prefix}grouphaikal`, buttonText: { displayText: '👥GROUP BOT👥' }, type: 1 },
...
case 'ppcp': 
simple.updatePresence(from, Presence.recording)
    anu = await fetchJson(`https://api.lolhuman.xyz/api/random/ppcouple?apikey=${humankey}`) 
			cowo = await getBuffer(anu.result.male)
			simple.sendMessage(from, cowo, image, {caption: 'Ini Untuk »  *𝗖𝗼𝘄𝗼𝗸𝗻𝘆𝗮'})
       cewe = await getBuffer(anu.result.female)
		    pll = `Ini Untuk » *𝗖𝗲𝘄𝗲𝗸𝗻𝘆𝗮*`
   sendButImage(from, pll, `Mysql Bot V3`, cewe, [                      
          {
...
//>>>>>>>>>>>>>>>>>>>>[ COMMAND ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
        ...
        rows: [
          {
            ...
            description: `Dalam tahap pengembangan`,
            rowId: `${prefix}${x}`
          }
        ]
      }
  ...
  case 'payment':
               ...
               listmsg(from, `${tampilUcapan}`,   `Hai kak *${pushname}*\n©mysqlbot, _Ini Command Payment Nya_`, list)
               break
...
[`sholawat1`,`sholawat2`,`sholawat3`,`sholawat4`,`sholawat5`,`sholawat6`,`sholawat7`,`sholawat8`,`sholawat9`,] 
               ...
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'Mysql Bot' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: ``,
               ...
               listmsg(from, `${tampilUcapan}`,   `Hai kak *${pushname}*\n©mysqlbot, _Ini Command Solawatnya_`, list)
               break
...
[`sound1`,`sound2`,`sound3`,`sound4`,`sound5`,`sound6`,`sound7`,`sound8`,`sound9`,`sound10`,`sound11`,`sound12`,`sound13`,`sound14`,`sound15`,`sound16`,`sound17`,`sound18`,`sound19`,`sound20`,`sound21`,`sound22`,`sound23`,`sound24`,`sound25`,]
               ...
               listmsg(from, `${tampilUcapan}`,   `Hai kak *${pushname}*\n©mysqlbot, _Ini Command Soundnya`, list)
               break
...
`_*HAI*_`
oy =`
https://chat.whatsapp.com/IoOsxcoDVd96vvFr0n1FUc
...
case 'original':
simple.updatePresence(from, Presence.recording)
img =  fs.readFileSync('./poto/YENI.jpeg')
teks = `Hai ${pushname} 👋${tampilUcapan}
I'M Z-BOT8 ^_^`
trans = `BOT INFO
🗿  _*Nama Bot : ${botname}*_
...
costum(buffer, sticker, Verived, `Gan`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
simple.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Gan' })
fs.unlinkSync(ran)
})
break
...
sendButton(from, "GOPAY : 085669011805", faketeks, but, mek)
break
case 'dana':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButton(from, "DANA : 085669011805", faketeks, but, mek)
break
case 'ovo':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButton(from, "OVO : 085669011805", faketeks, but, mek)
break

//━━━━━━━━━━━━━━━[ FITUR ASUPAN ]━━━━━━━━━━━━━━━━━//
case 'asupan':
    lahh = await fetchJson(`https://api.lolhuman.xyz/api/asupan?apikey=Z-BOT8`)
			lah = await getBuffer(lahh.result)
			await sendButVideo(from, `\nClick Button Next For More`, ``,lah, but = [{buttonId: `${prefix + command}`, buttonText: { displayText: `NEXT`}, type: 1}], hia = { quoted: mek, contextInfo: { mentionedJid: [sender]}})
			break
case 'cecan':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
simple.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Gan'})
break
case 'hijaber':
...
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
simple.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Gan'})
break
case 'ukhti':
             ...
             case 'silverplaybutton':
               ...
               simple.updatePresence(from, Presence.recording)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${humankey}&text=${ini_txt}`).then((gambar) => {
                        simple.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `©FWD IMAGE`, quoted : ftrol}) 
                    })
                    break
...
Sendbutdocument(from, tescok, "© MysqlBotV3*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`Z-BOT8.pdf`, pageCount: 9999999 },[{ buttonId: `${prefix}menu`, buttonText: { displayText: '🐣OWNER🐣' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://instagram.com/f.w.dalimunthe`}}})
break

case 'allmenu':
simple.updatePresence(from, Presence.recording)
teks =
`_Mau Allmenu Yang Mana Kak ${pushname}, Silahkan Di Pilih Kak,Yang Penting Jangan Spam_
...
`
but = [
{ buttonId: `${prefix}pdf`, buttonText: { displayText: 'ALL PDF'}, type: 1 },
{ buttonId: `${prefix}original`, buttonText: { displayText: 'ALL ORIGINAL'}, type: 1 },
]
sendButton(from, teks, faketeks, but) 
break
